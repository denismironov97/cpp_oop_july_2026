...
public:
    ...
    {
    private:
        long double latitude_coords;
        long double longitude_coords;
        string place_name;

    public:
        GeoLoc(long double latitude_coords, long double longitude_coords, string place_name);
    ...
    list<GeoLoc> getMatchesByCoords(const long double &latitude, const long double &longitude);

    // single match queries
    const GeoLoc *getSpecificMatchPlaceName(const string &place_name) const;
    const GeoLoc *getSpecificMatchCoords(const long double &latitude, const long double &longitude) const;

    // getters and setters
    const list<GeoLoc> getGeoBoxCollection() const;
};

GeoBox::GeoLoc::GeoLoc(long double latitude_coords, long double longitude_coords, string place_name)
{
...
const long double GeoBox::GeoLoc::getLatitude() const
{
    return this->latitude_coords;
}

const long double GeoBox::GeoLoc::getLongitude() const
{
...
const GeoBox::GeoLoc *GeoBox::getSpecificMatchCoords(const long double &latitude, const long double &longitude) const
{
...
list<GeoBox::GeoLoc> GeoBox::getMatchesByCoords(const long double &latitude, const long double &longitude)
{
    list<GeoBox::GeoLoc> matches_list;
    for (list<GeoBox::GeoLoc>::iterator curr_iter = this->geo_loc_box.begin(); curr_iter != this->geo_loc_box.end(); ++curr_iter)
    {
...
int main()
{
    ...
    while (std::getline(std::cin, curr_input_line) && curr_input_line != ".")
    {
        ...
        long double latitude = std::stold(latitude_str);
        long double longitude = std::stold(longitude_str);

        GeoBox::GeoLoc *ptr_geo_loc_obj = new GeoBox::GeoLoc(latitude, longitude, place_name);
    ...
    while (std::getline(std::cin, search_str_line) && search_str_line != ".")
    {
        ...
        if (is_float_num)
        {
            ...
            tmp_list = geo_box->getMatchesByCoords(curr_latitude, curr_longitude);
        }
        else
        {
